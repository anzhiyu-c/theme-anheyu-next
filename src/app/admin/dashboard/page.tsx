    ...
    <div className="space-y-6">
      ...
      >
        <div className="bg-card border border-border rounded-xl p-6 text-center">
          <p className="text-3xl font-bold text-primary">{formatNumber(stats.total.articles)}</p>
          <p className="text-sm text-muted-foreground mt-1">文章总数</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-6 text-center">
          <p className="text-3xl font-bold text-green-500">{formatNumber(stats.total.comments)}</p>
          <p className="text-sm text-muted-foreground mt-1">评论总数</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-6 text-center">
          <p className="text-3xl font-bold text-orange-500">{formatNumber(stats.total.views)}</p>
          <p className="text-sm text-muted-foreground mt-1">总浏览量</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-6 text-center">
          <p className="text-3xl font-bold text-purple-500">{formatNumber(stats.total.visitors)}</p>
