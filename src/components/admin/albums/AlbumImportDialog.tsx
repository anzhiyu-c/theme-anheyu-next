...
const PANEL_CLASS =
  "rounded-lg border border-[#f0f0f0] bg-white p-4 shadow-[0_2px_8px_rgba(0,0,0,0.06)]";
...
export default function AlbumImportDialog({ isOpen, onClose }: AlbumImportDialogProps) {
  ...
  const defaultCategoryOptions = useMemo(
    () => [{ key: "__none__", label: "不设置默认分类" }, ...categories.map(category => ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} size="4xl" scrollBehavior="inside">
      <ModalContent className="border border-[#f0f0f0] bg-white shadow-[0_12px_32px_rgba(0,0,0,0.12)]">
        <ModalHeader className="pb-2 border-b border-[#f0f0f0]">
          <div className="w-full space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-1">
                <p className="text-lg font-semibold tracking-tight text-[#1f1f1f]">导入相册</p>
                <p className="text-xs text-[#8c8c8c] leading-5">
                  支持链接、JSON 与压缩包导入。界面仅保留关键配置，提升批量导入效率。
                </p>
              </div>
              <Chip
                variant="flat"
                size="sm"
                className="h-7 px-2 border border-[#91caff] bg-[#e6f4ff] text-[#1677ff]"
              >
                100 条链接 / 50MB 文件
              </Chip>
            </div>
                ...
                <div className="rounded-lg border border-[#ffccc7] bg-[#fff2f0] p-3.5">
                  ...
                  <div className="max-h-44 overflow-y-auto space-y-2">
                    {result.errors.map((item, index) => (
                      <div key={`${item}-${index}`} className="rounded-md border border-[#ffccc7] bg-white p-2 text-xs text-[#a8071a]">
                        {item}
                      </div>
                    ))}
                  </div>
