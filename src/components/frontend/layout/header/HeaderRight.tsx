...
export function HeaderRight({
  ...
  const userAvatar = useMemo(() => {
    return user?.avatar || `https://cravatar.cn/avatar/${user?.email}?s=200&d=mp`;
  }, [user]);
    ...
    <div className={cn(styles.headerRight, isConsoleOpen && styles.consoleOpen)}>
      ...
      {!isMobile && (
        ...
        >
          <button className={styles.navButton} onClick={handleRandomArticle}>
            <Icon icon="ph:dice-five-fill" width="1.5rem" height="1.5rem" />
          </button>
        </Tooltip>
      )}
      ...
      >
        <button className={styles.navButton} onClick={handleSearchClick} aria-label="搜索">
          <Icon icon="fa7-solid:search" width="1.5rem" height="1.5rem" />
        </button>
      </Tooltip>
